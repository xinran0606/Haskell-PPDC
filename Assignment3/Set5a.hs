...
data BusTicket = SingleTicket | MonthlyTicket [Char]
...
totalPrice (MkShoppingEntry _ x xs) = x* fromIntegral xs
...
buyOneMore (MkShoppingEntry y x xs)= MkShoppingEntry y x (xs+1)
